...
timestamp=`date -j -u +"%Y%m%d_%H:%M:%S"`
...
echo "$timestamp UTC" > ../download/tsv/short_1d21.tsv.timestamp
...
echo "$timestamp UTC" > ../download/tsv/multi.tsv.timestamp
...
echo "$timestamp UTC" > ../download/tsv/mpfr.tsv.timestamp
...
echo "$timestamp UTC" > ../download/tsv/medical.tsv.timestamp
...
echo "$timestamp UTC" > ../download/tsv/jur.tsv.timestamp
...
echo "$timestamp UTC" > ../download/tsv/short_1pwmnm.tsv.timestamp
